...
Ba chuyện ở đây sai thì hỏng im lặng, không có traceback nào để lần:

  1. GHI ĐÈ .env LÀM MẤT CHÚ THÍCH. File .env của dự án có chú thích giải
     thích từng tham số. Dựng lại file từ một dict là xóa sạch phần đó, và
     không ai nhận ra cho tới lúc cần đọc.
  2. GHI SAI TÊN BIẾN. Vài trường nhận nhiều tên (TECHNICAL_ALERT_AFTER còn
     ăn TECHNICAL_RETRY_MAX). Ghi mù theo tên chính trong khi file đang dùng
     tên cũ thì .env có hai dòng cho cùng một tham số.
  3. GÁN VÀO BỘ NHỚ TRƯỚC KHI GHI ĐĨA. Đĩa đầy hoặc file bị khóa thì hệ
     thống chạy bằng cấu hình mới còn file giữ cấu hình cũ — khởi động lại là
     im lặng quay về giá trị cũ.

Không test nào ở đây động vào .env thật: mọi lần ghi đều vào tmp_path.
"""
...
def tra_lai_settings():
    """`settings` là object DÙNG CHUNG cả phiên chạy. Test nào đổi mà không
    trả lại thì mọi test sau chạy trên cấu hình khác — kiểu rò rỉ chỉ lộ ra
    khi đổi thứ tự test."""
...
def test_khoa_bi_mat_KHONG_nam_trong_danh_sach_sua_duoc():
    ...
    ngược vào .env dạng chữ thường — đúng thứ vừa bỏ công dọn đi."""
...
def test_CONFIG_GROUPS_phu_DU_moi_truong_sua_duoc():
    ...
    mất khỏi giao diện, không báo lỗi gì. Test này là thứ duy nhất bắt được."""
...
def test_gia_tri_cho_chon_deu_hop_le():
    """Danh sách chọn mà lệch với thứ config.py nhận thì người dùng chọn xong
    lưu không được, hoặc tệ hơn là lưu được một giá trị vô nghĩa."""
...
def test_khong_cho_nao_doc_settings_o_muc_module():
    """Nền tảng của cả tính năng này: sửa `settings` lúc đang chạy CHỈ có tác
    dụng nếu mọi nơi đọc lại trong thân hàm. Ai đó viết
    `NGUONG = settings.technical_alert_after` ở mức module là từ đó nút Lưu
    im lặng mất tác dụng cho riêng chỗ ấy."""
...
def test_sua_DUNG_dong_dang_dung_ten_cu(tmp_path):
    """`.env` đang dùng tên cũ TECHNICAL_RETRY_MAX. Phải sửa đúng dòng đó,
    không được thêm dòng TECHNICAL_ALERT_AFTER thứ hai — hai dòng cho một
    tham số thì người đọc sau không biết dòng nào có tác dụng."""
...
def test_gia_tri_co_dau_cach_hoac_thang_thi_boc_nhay():
    """python-dotenv coi phần sau `#` là chú thích, nên không bọc nháy là
    lặng lẽ mất đuôi giá trị."""
...
def test_gia_tri_sai_thi_KHONG_doi_gi_ca(env_gia):
    """Ba trường, trường thứ ba sai. Hai trường đầu không được phép đã kịp
    đổi — nửa cấu hình mới là kiểu hỏng không ai nghĩ tới lúc đọc log."""
...
def test_ghi_dia_THAT_BAI_thi_settings_khong_doi(env_gia, monkeypatch):
    """Thứ tự bắt buộc: ghi đĩa trước, gán bộ nhớ sau. Làm ngược thì khi đĩa
    đầy, hệ thống chạy cấu hình mới còn file giữ cấu hình cũ — khởi động lại
    là im lặng quay về giá trị cũ mà không ai hiểu vì sao."""
    truoc = settings.poll_interval_seconds
...
def test_so_ghi_hong_KHONG_lam_hong_viec_luu(env_gia, monkeypatch):
    """Mất cuốn sổ thì tiếc, nhưng không được phép làm hỏng chính việc lưu."""
