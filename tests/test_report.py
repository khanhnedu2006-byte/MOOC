...
def test_moi_tieu_che_sai_deu_sinh_dung_mot_chuoi():
    ...
    for mong_doi, truong in ca.items():
        ly_do = pipeline._mismatch_reason(
            ExtractedInfo(**truong), nhap, "test")
        assert mong_doi in ly_do
        khac = [c for c, _ in report.REJECTION_CAUSES if c != mong_doi]
        for c in khac:
            assert c not in ly_do, f"{truong} lẽ ra chỉ sai {mong_doi}, nhận {ly_do!r}"
...
def lich(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "FILE_TRANG_THAI", tmp_path / "s.json")
    ...
    return lambda khi: scheduler.kiem_tra_va_gui(
        datetime.fromisoformat(khi), gui_that=False)
...
def test_khoang_bao_cao_cua_ky_gui_bu_van_dung(lich, monkeypatch):
    ...
    tu, den, khoa = scheduler._khoang_bao_cao(
        "weekly", datetime.fromisoformat("2026-08-21T18:00"))
    assert (tu, den) == ("2026-08-15", "2026-08-21")
    assert khoa == "weekly:2026-W34"
...
def test_monthly_bao_cao_thang_TRUOC(lich, monkeypatch):
    ...
    tu, den, _ = scheduler._khoang_bao_cao(
        "monthly", datetime.fromisoformat("2026-09-05T10:00"))
    assert (tu, den) == ("2026-08-01", "2026-08-31")
...
def test_gui_hong_khong_thu_lai_lien_tuc(lich, monkeypatch, tmp_path):
    ...
    for i in range(12 * 60):                       # 1 giờ, mỗi 5 giây
        scheduler.kiem_tra_va_gui(t0 + timedelta(seconds=5 * i))
...
def test_gui_lai_duoc_sau_khi_loi_tam_thoi_het(lich, monkeypatch):
    ...
    def gui(*a, **k):
        if (moc[0] - t0) < timedelta(minutes=10):
            raise RuntimeError("mạng chập")

    fake = types.ModuleType("send_report")
    fake.gui_bao_cao_ky = gui
    monkeypatch.setitem(sys.modules, "send_report", fake)

    da_gui = None
    for i in range(12 * 60):
        moc[0] = t0 + timedelta(seconds=5 * i)
        if (k := scheduler.kiem_tra_va_gui(moc[0])):
            da_gui = k
            break
...
def test_daily_bao_cao_7_ngay_de_bieu_do_co_nghia(lich, monkeypatch):
    ...
    assert (tu, den) == ("2026-08-14", "2026-08-20")
    assert khoa == "daily:2026-08-20", "khóa phải theo NGÀY -> đúng 1 thư/ngày"


def test_bucket_qua_tho_bi_ha_xuong(lich):
    """REPORT_BUCKET=week với kỳ 7 ngày cho ra đúng một cột — phải tự hạ."""
    assert scheduler._bucket_hop_ly("week", "2026-08-14", "2026-08-20", "daily") == "day"
    assert scheduler._bucket_hop_ly("month", "2026-08-14", "2026-08-20", "daily") == "day"
    # Kỳ đủ dài thì giữ nguyên.
    assert scheduler._bucket_hop_ly("week", "2026-06-01", "2026-08-31", "monthly") == "week"
    assert scheduler._bucket_hop_ly("day", "2026-08-14", "2026-08-20", "daily") == "day"
    # Giá trị lạ không được làm chết job.
    assert scheduler._bucket_hop_ly("nam", "2026-08-14", "2026-08-20", "daily") == "day"
